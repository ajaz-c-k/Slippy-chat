...
export const Chatspages = ({ currentUser, onLogout }: ChatPageProps) => {
  ...
  // Effect for handling Socket.IO connections and events
  useEffect(() => {
    // Determine the backend URL using an environment variable
    // This will be set on Render to your backend service's public URL
    // For local development, it falls back to your local Flask server.
    const backendUrl = import.meta.env.VITE_REACT_APP_API_URL || "http://localhost:5000";

    // Initialize Socket.IO connection with the dynamic backend URL
    ...
    <div className="h-screen bg-gradient-background flex text-foreground"> {/* Changed to bg-gradient-background to use CSS variable */}
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="p-4 border-b border-border bg-card/50 backdrop-blur-sm">
          ...
          {/* User count and Slippy status - uses font-press-start */}
          <p className="text-xs text-muted-foreground font-press-start">
            {users.filter((u) => u.online).length} users online • Slippy is watching 👁
          </p>
        </div>
