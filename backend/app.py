...
#         # Only run if there's at least one user and some chat history
#         if len(users) < 1 or len(messages) < 2:
#             log("Slippy: Not enough users or messages to trigger.")
#             continue
...
#         if not recent_pool:
#             log("Slippy: No suitable recent messages found in pool.")
#             continue # No suitable messages to pick from

#         picked = random.choice(recent_pool) # Pick a random message from the filtered pool
#         log(f"Slippy: Picked message: '{picked['content']}' from '{picked['username']}'")

#         term = pick_keyword(picked["content"]) # Extract a keyword from the picked message
#         if not term:
#             log(f"Slippy: No suitable keyword found in '{picked['content']}'.")
#             continue # No suitable keyword found

#         log(f"Slippy: Picked term: '{term}'")

#         # Try to get a sentence from Urban Dictionary, then fallback to Wikipedia
#         line = get_urban_sentence(term) or get_wiki_sentence(term)
#         if not line or profanity.contains_profanity(line): # Ensure the fetched line is not empty or profane
#             log(f"Slippy: No clean or valid line found for term '{term}'.")
#             continue

#         log(f"Slippy: Found line: '{line}'")

#         # Choose a random "actor" (user) to send the "meddled" message as
#         candidates = [u for u in users.values() if u["username"] != picked["username"]]
#         if not candidates:
#             log("Slippy: No other users available to impersonate.")
#             continue # No other users available to impersonate
#         actor = random.choice(candidates)
#         log(f"Slippy: Impersonating '{actor['username']}'")
...
#             del messages[:-MAX_MSGS] # Trim history if it exceeds max limit
#         socketio.emit("chat", chaos_msg) # Emit the message to all connected clients
#         log(f"Slippy: Emitted message: '{chaos_msg['content']}' as '{chaos_msg['username']}'")
...
#     avatar = (data or {}).get("avatar", "") # Get avatar
#     users[request.sid] = {"username": username, "avatar": avatar} # Store user info with their session ID
#     log(f"User joined: {username} (SID: {request.sid})")

#     # Send the full list of currently online users to everyone
#     socketio.emit("users", list(users.values()))
#     log(f"Current users: {list(users.values())}")
...
#     messages.append(join_msg) # Add to history
#     socketio.emit("chat", join_msg) # Emit to all clients
#     log(f"System message: {join_msg['content']}")
...
#         del messages[:-MAX_MSGS] # Trim history
#     socketio.emit("chat", msg) # Emit to all clients
#     log(f"User message from {me['username']}: {content}")
...
#     if u:
#         socketio.emit("users", list(users.values())) # Update user list for everyone
#         log(f"User disconnected: {u['username']} (SID: {request.sid})")
#         leave_msg = {
#             "username": "System",
#             "avatar": "⛔", # System avatar for leaving
#             "content": f"{u['username']} left the room.",
...
#         socketio.emit("chat", leave_msg) # Emit to all clients
#         socketio.emit("user_left", u["username"]) # Emit specific event for frontend to handle user status
#         log(f"System message: {leave_msg['content']}")
...
# DEBUG variable and log() function are removed for direct print() calls
# DEBUG = True
# def log(*args, **kwargs):
#     if DEBUG:
#         print("[slippy:urban]", *args, **kwargs)
...
def get_urban_sentence(term: str):
    ...
    try:
        r = requests.get(url, params=params, timeout=6, headers=headers)
        if r.status_code != 200:
            print(f"[slippy:urban] Urban Dictionary API error: {r.status_code}") # Direct print
            return None

        payload = r.json()
        items = payload.get("list", [])
        if not items:
            print(f"[slippy:urban] No Urban Dictionary results for '{term}'") # Direct print
            return None
        ...
        for entry in random.sample(items[:min(len(items), 5)], min(len(items), 5)):
            ...
            if not profanity.contains_profanity(line):
                print(f"[slippy:urban] Found clean urban definition for '{term}': {line}") # Direct print
                return line

        print(f"[slippy:urban] No clean urban definition found for '{term}' after checking multiple entries.") # Direct print
        return None

    except (requests.RequestException, ValueError) as e:
        print(f"[slippy:urban] Request/Parsing error for Urban Dictionary: {e}") # Direct print
        return None
    except Exception as e:
        # Catch any other unexpected errors and log them
        import traceback
        print("[slippy:urban] An unexpected error occurred in get_urban_sentence:") # Direct print
        traceback.print_exc()
        return None
...
def get_wiki_sentence(term: str):
    ...
    try:
        ...
        if not profanity.contains_profanity(summary):
            return summary[:240] # Truncate and return if clean
        print(f"[slippy:urban] Wikipedia summary for '{term}' contains profanity.") # Direct print
        return None
    except wikipedia.exceptions.DisambiguationError as e:
        print(f"[slippy:urban] Wikipedia disambiguation error for '{term}': {e.options}") # Direct print
        return None
    except wikipedia.exceptions.PageError:
        print(f"[slippy:urban] Wikipedia page not found for '{term}'.") # Direct print
        return None
    except Exception as e:
        print(f"[slippy:urban] An unexpected error occurred in get_wiki_sentence for '{term}': {e}") # Direct print
        return None
...
def slippy_loop():
    ...
    while True:
        ...
        # Only run if there's at least one user and some chat history
        if len(users) < 1 or len(messages) < 2:
            print("[slippy:urban] Slippy: Not enough users or messages to trigger.") # Direct print
            continue
        ...
        if not recent_pool:
            print("[slippy:urban] Slippy: No suitable recent messages found in pool.") # Direct print
            continue # No suitable messages to pick from

        picked = random.choice(recent_pool) # Pick a random message from the filtered pool
        print(f"[slippy:urban] Slippy: Picked message: '{picked['content']}' from '{picked['username']}'") # Direct print

        term = pick_keyword(picked["content"]) # Extract a keyword from the picked message
        if not term:
            print(f"[slippy:urban] Slippy: No suitable keyword found in '{picked['content']}'.") # Direct print
            continue # No suitable keyword found

        print(f"[slippy:urban] Slippy: Picked term: '{term}'") # Direct print

        # Try to get a sentence from Urban Dictionary, then fallback to Wikipedia
        line = get_urban_sentence(term) or get_wiki_sentence(term)
        if not line or profanity.contains_profanity(line): # Ensure the fetched line is not empty or profane
            print(f"[slippy:urban] Slippy: No clean or valid line found for term '{term}'.") # Direct print
            continue

        print(f"[slippy:urban] Slippy: Found line: '{line}'") # Direct print

        # Choose a random "actor" (user) to send the "meddled" message as
        candidates = [u for u in users.values() if u["username"] != picked["username"]]
        if not candidates:
            print("[slippy:urban] Slippy: No other users available to impersonate.") # Direct print
            continue # No other users available to impersonate
        actor = random.choice(candidates)
        print(f"[slippy:urban] Slippy: Impersonating '{actor['username']}'") # Direct print
        ...
        if len(messages) > MAX_MSGS:
            del messages[:-MAX_MSGS] # Trim history if it exceeds max limit
        socketio.emit("chat", chaos_msg) # Emit the message to all connected clients
        print(f"[slippy:urban] Slippy: Emitted message: '{chaos_msg['content']}' as '{chaos_msg['username']}'") # Direct print
...
def on_join(data):
    ...
    avatar = (data or {}).get("avatar", "") # Get avatar
    users[request.sid] = {"username": username, "avatar": avatar} # Store user info with their session ID
    print(f"[slippy:urban] User joined: {username} (SID: {request.sid})") # Direct print

    # Send the full list of currently online users to everyone
    socketio.emit("users", list(users.values()))
    print(f"[slippy:urban] Current users: {list(users.values())}") # Direct print
    ...
    messages.append(join_msg) # Add to history
    socketio.emit("chat", join_msg) # Emit to all clients
    print(f"[slippy:urban] System message: {join_msg['content']}") # Direct print
...
def on_send(data):
    ...
    if len(messages) > MAX_MSGS:
        del messages[:-MAX_MSGS] # Trim history
    socketio.emit("chat", msg) # Emit to all clients
    print(f"[slippy:urban] User message from {me['username']}: {content}") # Direct print
...
def on_disconnect():
    ...
    if u:
        socketio.emit("users", list(users.values())) # Update user list for everyone
        print(f"[slippy:urban] User disconnected: {u['username']} (SID: {request.sid})") # Direct print
        leave_msg = {
            "username": "System",
            "avatar": "⛔", # System avatar for leaving
            "content": f"{u['username']} left the room.",
